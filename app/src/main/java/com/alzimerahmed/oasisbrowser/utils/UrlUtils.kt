...
fun smartUrlFilter(url: String, canBeSearch: Boolean, searchUrl: String): String {
    var inUrl = NavigationSecurity.sanitizeUserInput(url)
    ...
    if (matcher.matches()) {
        ...
        return if (NavigationSecurity.isAllowedTopLevelNavigation(inUrl)) inUrl else ""
    }
    if (!hasSpace) {
...
/**
 * Determines if the url is a url for the reading list page.
 *
 * @return true if the url is a reading list url, false otherwise.
 */
fun String?.isReadingListUrl(): Boolean =
    this != null && this.startsWith(FILE) && this.endsWith(ReadingListPageFactory.FILENAME)
